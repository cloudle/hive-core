(ns hive.utils.firebase.users
  (:require
    [hive.utils.firebase.tasks :refer [derefify continue-with]])
  (:import
...
(defn token->map [^FirebaseToken decoded-token]
  {:issuer         (.getIssuer decoded-token)
   :name           (.getName decoded-token)
   :email          (.getEmail decoded-token)
   :email-verified (.isEmailVerified decoded-token)
...
(defn verify-token [app token]
  (-> (FirebaseAuth/getInstance app)
      (.verifyIdToken token)
      (derefify)
      (continue-with token->map)))